...
import com.minisdk.glide.load.resource.transcode.UnitTranscoder;
import com.minisdk.glide.util.Synthetic;
import java.util.ArrayList;
import java.util.List;

/**
 * A class that allows {@link com.minisdk.glide.load.resource.transcode.ResourceTranscoder}s to be
...
public class TranscoderRegistry {
  ...
  /**
   * Registers the given {@link com.minisdk.glide.load.resource.transcode.ResourceTranscoder} using
  ...
  public synchronized <Z, R> void register(
      ...
      @NonNull com.minisdk.glide.load.resource.transcode.ResourceTranscoder<Z, R> transcoder) {
    transcoders.add(new Entry<>(decodedClass, transcodedClass, transcoder));
  }

  /**
   * Returns the currently registered
   * {@link com.minisdk.glide.load.resource.transcode.ResourceTranscoder} for the given classes.
   *
  ...
  public synchronized <Z, R> com.minisdk.glide.load.resource.transcode.ResourceTranscoder<Z, R> get(
    ...
    if (transcodedClass.isAssignableFrom(resourceClass)) {
      return (com.minisdk.glide.load.resource.transcode.ResourceTranscoder<Z, R>) UnitTranscoder.get();
    }
    for (Entry<?, ?> entry : transcoders) {
      if (entry.handles(resourceClass, transcodedClass)) {
        return (com.minisdk.glide.load.resource.transcode.ResourceTranscoder<Z, R>) entry.transcoder;
      }
    }
  ...
  private static final class Entry<Z, R> {
    ...
    @Synthetic final com.minisdk.glide.load.resource.transcode.ResourceTranscoder<Z, R> transcoder;

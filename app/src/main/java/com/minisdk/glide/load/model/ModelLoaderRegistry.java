...
/**
 * Maintains an ordered put of {@link com.minisdk.glide.load.model.ModelLoader}s and the model and data types they handle in
...
public class ModelLoaderRegistry {
  ...
  private <Model, Data> void tearDown(
      @NonNull List<com.minisdk.glide.load.model.ModelLoaderFactory<? extends Model, ? extends Data>> factories) {
  ...
  @NonNull
  public <A> List<com.minisdk.glide.load.model.ModelLoader<A, ?>> getModelLoaders(@NonNull A model) {
    List<com.minisdk.glide.load.model.ModelLoader<A, ?>> modelLoaders = getModelLoadersForClass(getClass(model));
    int size = modelLoaders.size();
    boolean isEmpty = true;
    List<com.minisdk.glide.load.model.ModelLoader<A, ?>> filteredLoaders = Collections.emptyList();
    //noinspection ForLoopReplaceableByForEach to improve perf
    for (int i = 0; i < size; i++) {
      com.minisdk.glide.load.model.ModelLoader<A, ?> loader = modelLoaders.get(i);
  ...
  @NonNull
  private synchronized <A> List<com.minisdk.glide.load.model.ModelLoader<A, ?>> getModelLoadersForClass(
      @NonNull Class<A> modelClass) {
    List<com.minisdk.glide.load.model.ModelLoader<A, ?>> loaders = cache.get(modelClass);
    if (loaders == null) {
  ...
  private static class ModelLoaderCache {
    ...
    public <Model> void put(Class<Model> modelClass, List<com.minisdk.glide.load.model.ModelLoader<Model, ?>> loaders) {
    ...
    @Nullable
    @SuppressWarnings("unchecked")
    public <Model> List<com.minisdk.glide.load.model.ModelLoader<Model, ?>> get(Class<Model> modelClass) {
      Entry<Model> entry = (Entry<Model>) cachedModelLoaders.get(modelClass);
      return entry == null ? null : entry.loaders;
    }

    private static class Entry<Model> {
      @Synthetic final List<com.minisdk.glide.load.model.ModelLoader<Model, ?>> loaders;
